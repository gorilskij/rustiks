...
fn load1<P: AsRef<Path>, K>(path: P) -> HashMap<K, Tern<Vec<K>, Alg>>
where
    K: PieceKey + Eq + Hash + Debug,
{
    ...
    for line in iter_lines(path) {
        ...
        let mut tern_vec = Vec::<(Vec<K>, Alg)>::with_capacity(10);
        while !line.is_empty() {
            ...
            let end = line.index_of('"', 1).expect("line ended unexpectedly");
...
fn load2<P: AsRef<Path>>(path: P) -> HashMap<Vec<usize>, Alg> {
    ...
    for line in iter_lines(path) {
        ...
        let pred = pred
            ...
            .map(|i| {
                i.parse()
                    .unwrap_or_else(|_| panic!("Invalid value for usize: {}", i))
            })
            .collect();
        let alg = Alg::from(&alg[1..alg.len() - 1]);
...
lazy_load!(
    ...
    cross_data,
    load1,
    "src/algorithm_data/data/cross.txt",
    MapToTern<Pos<2>>
);
...
lazy_load!(
    ...
    f2l_data,
    load1,
    "src/algorithm_data/data/f2l.txt",
    MapToTern<CEPos>
);
...
lazy_load!(
    ...
    oll_data,
    load2,
    "src/algorithm_data/data/oll.txt",
...
lazy_load!(
    ...
    pll_data,
    load2,
    "src/algorithm_data/data/pll.txt",
    HashMap<Vec<usize>, Alg>
);
